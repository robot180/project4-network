...
function editpost(content) {
  ...
  // hide original post content and the edit button:
  document.getElementById(`postBody_${content.editpostId}`).style.display = "none";
...
async function submit_edits(content, editedpost) {
  console.log(content)
...
.then(result => {
    ...
    // updated_postdiv.innerHTML = `
    // <div class="debug row px-3">
    // <div class="debug mr-auto col-sm-8  p-2 order-md-2 small text-left text-muted font-italic font-weight-lighter align-self-center">
    //     Posted On: ${updated_post.timestamp} by <a href="${link}" class="user-link">${content.postauthor}</a>
    // </div>
    // <div  class="debug col-3 col-sm-4 col-md-2 p-2 order-md-2 text-right font-italic">
    //   <strong id="post{{post.id}}-likes">Likes: ${updated_post.liked_by.length}</strong>
    // </div>
    // </div>
    // <div class="mb-2 col px-2 pb-2 pt-md-2 order-md-1 text-break"> 
    // ${updated_post.title}
    // <small><em>${updated_post.body}</em></small>
    // </div>`;
    ...
    document.getElementById(`post${result[0].pk}editpost`).style.display = "block";
...
async function follow(data) {
  ...
  document.getElementById("followers").innerHTML = `<strong>Followers: ${newfollowersCount}</strong>`;
